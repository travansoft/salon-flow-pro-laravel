...
@section('title', 'Backfill Old Bill')

@section('content')
    <div class="sfp-page-header">
        <div>
            <h1 class="sfp-page-title">Backfill old bill</h1>
            <p class="sfp-page-subtitle">Enter a bill for a past date. Rates are editable, and the bill is recorded as paid immediately.</p>
        </div>
    </div>
    ...
    <div class="sfp-card">
        <form id="backfill-form">
            ...
            <div class="sfp-field">
                <label class="sfp-label">Line items <span style="color:#94A19D;font-weight:400">(rate is editable)</span></label>
            ...
            <div class="sfp-form-actions">
                <button type="button" id="bill-settle" class="sfp-btn-primary">Create backdated bill</button>
...
(function () {
    ...
    async function createBackfilledBill() {
        if (!validateLines()) {
            return;
        }
        ...
        try {
            const response = await fetch('{{ $tenantUrl->route("bills.backfillStore") }}', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                if (response.status === 422 && data.errors) {
                    ...
                    setFeedback(firstError || 'Could not save the bill.', true);
                } else {
                    setFeedback(data.message || 'Could not save the bill.', true);
                }
            ...
            setFeedback('Network error saving the bill.', true);
            settleBtn.disabled = false;
        }
    }

    settleBtn.addEventListener('click', createBackfilledBill);
